...
def eval_errors(sol, model):
    ...
    for i in range(4):
        err = 0
        for p_i in points:
            data = data_all[i, int((p_i[0]+1)*2/len(data_all[0]))]
...
if __name__ == "__main__":
    ...
    for i in 50*np.array([2**i for i in range(3,10)]):
        ...
        print(x.shape)
        #c = x*0
        c = np.array(data[::int(len(data)/i)]).reshape((i,1))
                    
        
        #for ii in range(i):
        #    c[ii][0] = data[::int(len(data)/i)]#sol.eval(x[ii], [0], func=1)
        #print(c)
        ...
        for j in range(n):
            ...
            for long_iteration in range(5):
                data_filename = 'raw_data/I_data.pkl'#+str(i)+'_'+str(j)+'.pkl'
                ...
                for iteration in range(1):
                    ...
                    #data_filename = 'raw_data/I_data'+str(i)+'_'+str(j)+'.pkl'
