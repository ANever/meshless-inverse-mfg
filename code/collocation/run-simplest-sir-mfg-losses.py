...
def eval_error(sol, sol_mes, A,b):
    ...
    er[4] = 0#abs(sol.eval([0.2],[0],func=4)-20)
...
def eval_error_rel(sol, sol_mes, A,b):
    ...
    for func in range(4):
        for i, t in enumerate(ts):
            val = (1 + fixed_noize[i])*sol_mes.eval([t],[0],func)
            inc = (sol.eval([t],[0],func) - val)/(np.abs(val) + 1e-10)
            er[func] += abs(float(inc))**2
        er[func] = np.sqrt(er[func])
        er[4] = 0#abs((sol.eval([0.1],[0],func=4) + sol.eval([0.5],[0],func=4) + sol.eval([0.99],[0],func=4))/3-20)/20
...
for i, beta_max in enumerate(np.linspace(5,40,n_trials)):
            ...
            out_string = str(beta_max) #str(noise_lvl) + ',' + str(num_data_points)
            ...
            for er in rel_errors:
                out_string += ',' + str(er)
            out_string +='\n'
            with open('result-direct-noisy1.csv', 'a') as f:
                f.write(out_string)
